...
class TestTeemClient(TestCase):
    ...
    def test_teem_client_send(self, patched):
        ...
        assert patched.call_args.kwargs['url'] == 'https://product.apis.f5.com/ee/v1/telemetry'
        assert patched.call_args.kwargs['headers']['User-Agent'] == 'F5_BIGIP/{0}'.format(CURRENT_COLL_VERSION)
        assert patched.call_args.kwargs['headers']['F5-ApiKey'] == TEEM_KEY
        assert patched.call_args.kwargs['data']['digitalAssetVersion'] == CURRENT_COLL_VERSION
        assert patched.call_args.kwargs['data']['observationStartTime'] == self.start_time
    ...
    def test_teem_client_send(self, patched):
        ...
        self.connection.httpapi._display_message = MagicMock()
        patched.side_effect = HTTPError(
            'https://product.apis.f5.com/ee/v1/telemetry', 400, load_fixture('teem_error_response.json'), {}, None
        )

        teem = TeemClient(self.client, self.start_time)
        with self.assertRaises(HTTPError) as exc:
            teem.send()
        assert exc.exception.code == 400
        assert exc.exception.reason == {'errors': [{'id': '5b26d040-bc74-11ea-a12c-efb44ca2c609', 'code': 'bad-request',
                                        'code2': '', 'message': 'Invalid request: epochTime must be a number'}]
                          }