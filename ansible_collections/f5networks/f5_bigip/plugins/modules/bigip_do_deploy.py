...
class ModuleParameters(Parameters):
    ...
    def timeout(self):
        ...
        if timeout < 300 or timeout > 3600:
            raise F5ModuleError(
                "Timeout value must be between 600 and 3600 seconds."
            )
...
class ModuleManager(object):
    ...
    def async_wait(self, task_id):
        period, interval = self.want.timeout
        task = self.wait_for_task(task_id, period, interval)
        if task:
    ...
    def wait_for_task(self, task, period, interval):
