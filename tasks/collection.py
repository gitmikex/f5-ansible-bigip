...
@task(name='change-version', help=dict(
    version="What version of collection to set in the galaxy.yml file, the version must follow in SemVer format.",
    collection="The collection name for which the galaxy.yml file to be changed.",
))
def change_galaxy_version(c, version, collection):
...
@task(optional=['collection'], help=dict(
    ...
    collection="The collection name to which the modules are upstreamed.",
    update="Allows updating galaxy file when requested."
))
def build(c, version, collection, update=True):
